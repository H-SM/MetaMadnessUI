...
const ExploreCard = ({id, imgUrl, title, index, active, handleClick}) => (
  ...
  onClick={() => handleClick(id)}>
    ...
    {active !== id ? (
      <h3 className='font-semibold sm:text-[25px] text-[18px] text-white absolute z-0 lg:bottom-20 lg:rotate-[-90deg] lg:origin-[0,0]'>{title}</h3>
    ): (
      <div className='absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]'>
        <div className={`${styles.flexCenter} w-[60px] h-[60px] rounded-[24px] glassmorphism mb-[16px]`}>
          <img src="/headset.svg" alt="headset" className='w-1/2 h1/2 object-contain'/>
        </div>
